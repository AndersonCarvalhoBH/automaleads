...
import {
  ...
  LeadImportItem,
  apiListUsers,
  apiCreateUserForAccount,
  apiUpdateUserForAccount,
  apiDeleteUserForAccount,
  UserDto,
} from "../api";
import {
  apiGetMyAccount,
...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  async function fetchCepAddress(cepValue: string) {
    ...
    try {
      ...
      toast({
        title: "CEP invalido ou n�o encontrado",
  ...
  async function loadLeads() {
    try {
      ...
      toast({
        ...
        description:
          "N�o foi poss�vel buscar os leads no backend. Verifique a API_URL e o token.",
        status: "error",
        duration: 4000,
        isClosable: true,
      });
  ...
  async function loadUsers() {
    try {
      ...
      setUsersError(err?.message || "Erro ao listar usuarios");
    } finally {
  ...
  async function handleCreateLead(e: React.FormEvent) {
    ...
    try {
      ...
      toast({
        ...
        description:
          error.message || "N�o foi poss�vel criar o lead no backend.",
        status: "error",
        duration: 4000,
        isClosable: true,
      });
  ...
  async function handleSaveProfile(e: React.FormEvent) {
    ...
    try {
      ...
      toast({
        ...
        description:
          "Suas informa��es de conta e faturamento foram atualizadas.",
        status: "success",
        duration: 2500,
      });
    } catch (err: any) {
    ...
    return (
      <Box>
        ...
        <StatsGrid>
          ...
          />
          <StatCard
            label="Taxa de convers�o (simulada)"
            value={`${taxaConversao}%`}
                  ...
                  <Tr key={lead.id}>
                    ...
                    <Td>
                      <Tag size="sm" colorScheme="blue" variant="subtle">
                        <TagLabel>
                          {lead.source || lead.origem || "N�o informado"}
                        </TagLabel>
                      </Tag>
                    </Td>
                  </Tr>
  ...
  function renderUsersSection() {
    return (
      <Box>
        <PageHeader title="Usuarios" description="Gerencie usuarios desta conta." />

        <Flex justify="space-between" align="center" mb={3}>
          <Box />
          <Button size="sm" colorScheme="blue" onClick={() => { resetUserForm(); onOpenUserModal(); }}>
            + Usuário
          </Button>
        </Flex>
        ...
        <Box borderWidth="1px" borderColor="gray.100" bg="white" borderRadius="lg" p={4} boxShadow="md">
          {loadingUsers ? (
            <Flex align="center" gap={2}>
              <Spinner size="sm" />
              <Text fontSize="sm">Carregando usuarios...</Text>
            </Flex>
          ) : users.length === 0 ? (
              ...
              <Tbody>
                {users.map((u) => (
                  <Tr key={u.id}>
                    ...
                    <Td>{u.email}</Td>
                    <Td>{u.role}</Td>
                    <Td>{u.createdAt ? formatarData(u.createdAt) : "-"}</Td>
                    <Td textAlign="right">
                      <HStack justify="flex-end" spacing={2}>
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              await apiDeleteUserForAccount({ token, id: u.id });
                              toast({ title: "Usuario removido", status: "success" });
                              loadUsers();
                            } catch (err: any) {
                              toast({
        ...
        <Modal isOpen={isUserModalOpen} onClose={() => { onCloseUserModal(); resetUserForm(); }} isCentered>
          <ModalOverlay />
          <ModalContent>
            <ModalHeader>{editingUser ? "Editar usuario" : "Novo usuario"}</ModalHeader>
            <ModalCloseButton />
            <ModalBody>
              <Stack spacing={3}>
                <FormControl isRequired>
              ...
              <Button
                ...
                onClick={async () => {
                  try {
                    ...
                    if (editingUser) {
                      await apiUpdateUserForAccount({
                        token,
                        id: editingUser.id,
                        name: userForm.name,
                        role: userForm.role,
                      });
                      toast({ title: "Usuario atualizado", status: "success" });
                    } else {
                      await apiCreateUserForAccount({
                        token,
                        name: userForm.name,
                      ...
                      toast({ title: "Usuario criado", status: "success" });
                    }
                    onCloseUserModal();
                    resetUserForm();
                    loadUsers();
        ...
        <Stack spacing={6} opacity={loadingProfile ? 0.6 : 1}>
            ...
            <Flex gap={6} align="center" mb={4} flexWrap="wrap">
              <Box textAlign="center">
                <Avatar
                  ...
                  name={profileName || currentUserName || "Usu�rio"}
                  bg="purple.500"
                  color="white"
                  mb={2}
                />
          ...
          >
            ...
            <SimpleGrid columns={[1, 3]} spacing={4} mb={4}>
              ...
              <FormControl>
                <FormLabel fontSize="sm">N�mero</FormLabel>
                <Input
                  size="sm"
